...
@app.post("/api/ingest/text-file", response_model=IngestTextFileResponse, status_code=status.HTTP_201_CREATED)
def ingest_text_file(req: IngestTextFileRequest) -> IngestTextFileResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Input text file not found") from exc
